...
| Authenticated calendar subscription | `src/calendar-adapter.ts`, `src/reservation-day.ts`, `src/worker.ts` | Calendar/Worker tests: exact capability token, uniform 404, CRLF UTF-8 ICS, stable opaque UID, create/update/removal, bounded projection/retention, no customer/contact/proof fields; `tests-browser/owner.spec.ts` verifies the owner-only diagnostics path and zero customer calendar trace |
| Optional Google calendar projection | `src/calendar-adapter.ts`, `src/reservation-day.ts`, `src/worker.ts` | Fixture-only tests: fixed OAuth/Calendar URLs, redirect/body bounds, deterministic event ID, update→insert/409→update convergence, delete 404/410, retry/claim races, terminal diagnostics, seven-day reconciliation, and byte-identical reservation availability during provider failure |
...
| Capability | Production task it serves | Status | Current evidence / remaining gap | Roadmap stage / exclusion boundary |
|---|---|---|---|---|
| ICS subscription feed (outbound, authenticated) | The operator sees bookings inside their own calendar | Implemented (optional adapter) | Stable opaque ICS projection behind an exact revocable capability, with uniform failures, bounded retention, and no booking/customer secret fields (`src/calendar-adapter.ts`, `src/worker.ts`; `test/calendar-adapter.test.ts`, `test/worker.test.ts`) | Stage S2 |
| Outbound calendar event synchronization (first implementation: Google, per issue #1's recorded decision) | Bookings appear as events in an external calendar | Implemented (optional adapter) | Fixed-host OAuth/Calendar writes, latest-desired durable queue, bounded retry, redacted terminal visibility, and owner reconciliation; provider failures never alter reservation availability (`src/calendar-adapter.ts`, `src/reservation-day.ts`, `src/worker.ts`; fixture tests only) | Stage S2 |
