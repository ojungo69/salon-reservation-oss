...
- [ ] T001 **Pre-implementation docs gate (blocks all code)**: re-verify against current Cloudflare docs — (a) Durable Object alarm behavior when a class/binding is removed (rollback scenario for `AdapterDelivery`) and whether stored state + pending alarms survive re-introduction, (b) the additive `new_sqlite_classes` migration deploy sequence and its rollback ordering, (c) alarm retry semantics (retry count/backoff bounds) that `ALARM_RETRY_BACKOFF_ALLOWANCE_S` must cover. Record findings with URLs as an appendix in specs/003-line-adapter/research.md; adjust constants if contradicted.
...
- [ ] T003 wrangler.jsonc: add `AdapterDelivery` DO binding + additive `new_sqlite_classes` migration (per T001 sequence); add `LINE_MESSAGING_CHANNEL_SECRET` to secrets **optional** docs (never `secrets.required`); run `npm run types` and commit regenerated worker-configuration.d.ts; `npm run types:check` green.
...
- [ ] T030 [US1] Message templates for the five events (approve/reject/reschedule/cancel/expire) — short polite Japanese, fragment-only storage, no retroactive delivery for pre-link events.
...
- [ ] T036 docs/PARITY.md + docs/ROADMAP.md: flip S1 row with evidence links; docs/RELEASING.md rollback note (disabled + drained gate; **never run a `deleted_classes` migration for `AdapterDelivery` on rollback** — the generation high-water's survival depends on storage persisting); docs/ADAPTER-CONTRACTS.md crosswalk references if drifted; release/public-files.txt += new public files; `npm run release:audit` green.
...
- [ ] T039 Task-completion verification (speckit-verify-tasks equivalent — run speckit-analyze + manual sweep): every `[X]` above has landed implementation + tests; then PR "Part of #1" (issue #1 stays open).
