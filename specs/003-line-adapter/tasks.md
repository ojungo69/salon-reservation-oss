...
- [X] T007 src/reservation-day.ts: `drainOutbox` (bounded `OUTBOX_DRAIN_BATCH`, deletes nothing, existence-check-and-return on uninitialized days — creates no schema/alarm), idempotent generation-scoped `ackOutbox(event identities)`, `readEventSequence`.
...
- [X] T026 [US1] src/line-adapter.ts: stateless token mint (`POST /oauth2/v3/token`, cache `TOKEN_CACHE_TTL_S`, keyed generation + channel ID + secret discriminator; 408/429/5xx/timeout retryable, other non-200 responses configuration-rejected) + push client (`POST /v2/bot/message/push`, persisted `X-Line-Retry-Key` minted before first attempt, byte-identical retries, only 5xx/timeout retryable, 409 accepted, 401 parked as `awaiting-configuration`, 429/other 4xx terminal, `redirect:"manual"`, wire-format v1 serializer — subject resolved at send from the link row).
...
- [X] T032 [US1] Pipeline tests in test/adapter-delivery.test.ts: accept → resolve → queue → push → terminal with fetchMock; sustained 5xx through the full retry ladder; delayed queued-send cutoff; timeout; 409-accepted; disposition matrix (all 7 branches, byte-identical store for branch 0 across accept/finalize/webhook); follow/unfollow ordering including expiry of an attempted parked row before re-queue; watermark ignored-prelink; awaiting-configuration under missing secret then recovery; overflow; deactivation saga incl. mid-saga death + re-drive and disable-time ledger-cap enforcement; clock-tested sweep cycle with a date larger than `OUTBOX_DRAIN_BATCH`, per-batch lateness + mid-batch death + parked pull + parked ack in one run.
...
- [X] T034 [P] docs/LINE-SETUP.md operator walkthrough: Mini App recommendation + LINE Login alternative (research R2), channel creation with placeholder identifiers only, secret via `wrangler secret put`, **the regional quota warning with Japan's 200-messages/month Communication Plan example** (research R3), live-channel verification steps (operator-side, never CI), rotation path.
