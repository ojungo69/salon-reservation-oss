...
- [X] T025 [US2] Tests in test/line-adapter.test.ts + test/worker.test.ts: signature fixtures (valid/invalid/oversized/bad-base64, constant-time path), verify fixtures (expired/wrong-audience/oversized/missing-field/type-mismatch/profile-claims-present, redirect fixtures cross-origin + same-origin), duplicate webhook delivery, replay posture (re-link no-op, conflict, theft-resistance: token alone insufficient without nonce), intent expiry, provisional expiry outcome, enable/disable/partial-config on existing days.
