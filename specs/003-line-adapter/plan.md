...
**Created**: 2026-08-12 · **Status**: Draft (plan gate pending)
...
| Area | Files | Change |
|---|---|---|
| Event foundation | `src/reservation-day.ts`, `src/adapter-delivery.ts` (new DO), `src/worker.ts`, `wrangler.jsonc`, `worker-configuration.d.ts` | `__adapter_outbox` + event sequence (lazily created, invisible to the legacy schema check — decision 4; **the day alarm and its handler are untouched**); post-commit handoff + lazy re-poke; `AdapterDelivery` DO (event accept/dedup, links, subject index, deliveries with persisted attempt/`nextAttemptAt`, webhook dedup, lifecycle authority, diagnostics, retention sweep — all timed work on its own reservation-core-isolated alarms); new DO binding + SQLite export **with an explicit additive `new_sqlite_classes` migration in `wrangler.jsonc`**; generated types refreshed via `npm run types` so `types:check` passes with the `AdapterDelivery` binding present |
...
| Routes | `src/worker.ts` | **Lifecycle**: `POST /api/admin/line/enable` (body: the three identifiers + `commandId` + `expectedLifecycleVersion`) and `POST /api/admin/line/disable` (body: `commandId` + `expectedLifecycleVersion`) — both owner-gated, same-origin, rate-limited like existing admin commands. A third owner-gated command, `POST /api/admin/line/settings` (body: the three identifiers + `commandId` + `expectedLifecycleVersion`), stores **draft identifiers while `disabled`** (the spec's partial-configuration states — IDs without secret, secret without IDs — are thus real, inspectable, reload-stable, and produce zero subjects, outbox rows, or outbound traffic). **All three lifecycle commands share one contract**: body carries `commandId` + `expectedLifecycleVersion` (+ the three identifiers for settings/enable), one receipt namespace with payload fingerprints, and **every mutation bumps the version** — so a stale settings replay after a later disable fails CAS and cannot overwrite the draft. **Enable's identifiers are authoritative**: they supersede any stored draft (the draft is a setup convenience, documented as such). The `commandId`+different-payload → 409 guarantee holds while the receipt is retained; after eviction the version CAS still makes every stale request non-mutating (both boundaries tested, plus concurrent settings-vs-enable). Enable refuses while the secret binding is absent (the saga never starts unstartable; the setup surface shows which half is missing). Idempotency is one fixed pipeline for all three commands: **receipt match first** (same `commandId` + same fingerprint → replay the original outcome; same `commandId` + different fingerprint → 409, a guarantee scoped to the receipt retention window) → **then the phase + `expectedLifecycleVersion` CAS** (after receipt eviction a stale request always fails CAS and is non-mutating — an old enable replayed against `disabled`, an old disable against `active`, an old settings after a later transition: all rejected; tested across the cap boundary) → only then does the command start. `lifecycleVersion` increments **exactly once per accepted external command** — internal saga re-drives never change it, so the client-visible CAS contract is stable. **Customer link**: `POST /api/reservations/:reservationId/line/link-intent` and `.../line/unlink` — reservation ID in the path, `{date, managementKey}` in the JSON body (the existing public-route convention), same-origin, rate-limited; intent minting touches `AdapterDelivery` only after the active check; unlink works with management proof alone whenever a link exists — even mid-provider-outage, under a missing secret, or while deactivating (it depends on no capability, SDK, or secret). Link visibility comes from a **separate proof-bound adapter status endpoint** (`POST /api/reservations/:reservationId/line/status`, same body/auth shape), fetched progressively by the management page — the existing status/management responses are **not** modified, so an adapter stall can never delay or fail them (tested: adapter stopped → existing status body, code, and latency unchanged). It returns presence only (no subject) when a link or residual link exists; with no link and no history it 404s like every LINE surface, leaving no trace. **The degraded-state path stays reachable**: while the public config carries the cleanup-only marker (missing-secret, `deactivating`), the opt-in module is still served and runs in **cleanup mode** — proof-bound status + unlink only, no LIFF, no LINE fetches — so a customer can always see and undo a link; never-configured and post-purge installations serve neither marker nor module (tested per state). `POST /api/adapters/line/link` (nonce + ID token; same-origin, JSON-only, rate-limited; nonce existence/TTL/generation pre-checked before any LINE fetch and re-checked inside the completing transaction); `POST /api/adapters/line/webhook` (404 while inactive); LIFF page served Worker-first covering `/line.html` **and every serving alias** (`/line`, trailing-slash variants — each tested inactive→404, active→same CSP); `privacy.html` served Worker-first with the LINE disclosure section rendered **only** in the states that can hold reservation-linked LINE data — `active`, missing-secret, and `deactivating` until purge completes (**the adapter state table is the single authority**; the post-purge redacted ledger and webhook-dedup IDs are operational, non-personal records — the permanent privacy text explains such operational records and their TTL generically, so the LINE section ends at purge; inactive DOM carries zero LINE trace; browser-tested in all four states: inactive / active / missing-secret / after disable-purge); admin diagnostics extension |
| Frontend | `public/line.html` (new), `public/line-optin.js` (new), `public/app.js`, `public/setup.html`, `public/admin.html`, `public/styles.css`, `public/_headers` | LIFF bootstrap page; the opt-in behavior lives in a **separate module** (`line-optin.js`) dynamically imported by `app.js` when the public config carries the **capability or the cleanup marker** (cleanup mode = proof-bound status + unlink only) — `app.js` gains only the inert guard; in `never-configured`, `draft-disabled`, `activating`, and `disabled-purged` the guard stays inert and the module (like `/line.html` and its aliases) is served 404 Worker-first, so absence is assertable at the network level: the module is never fetched and is unfetchable, zero LINE DOM, `/api/config` byte-identical (the state table is the single authority; spec FR-001's boundary and cleanup exception). A dynamic-import failure (disable racing the config fetch, network failure) degrades locally — the LINE affordance is omitted and the management journey continues untouched (browser tests: disable-after-config-fetch, module-fetch failure). Setup section + diagnostics; `/line.html`-scoped CSP override |
| Secrets | (no `wrangler.jsonc` change) | `LINE_MESSAGING_CHANNEL_SECRET` as an **optional binding on `AppEnv`** — `secrets.required` stays `[OWNER_TOKEN, TURNSTILE_SECRET]`; operator registers it via `wrangler secret put` (documented) |
| Tests | `test/line-adapter.test.ts`, `test/line-outbox.test.ts`, `test/line-delivery.test.ts` (new), `tests-browser/line.spec.ts` (new), `vitest.config.ts`, `package.json`, `playwright.config.ts` | **No existing test file is edited.** New files added to the vitest include list / scripts; fixture bindings for channel IDs + secret. **Layered fixture strategy** — Playwright route interception covers only browser-originated requests, so: the full backend journey (verify/token/push server-side fetches) is E2E-tested in Vitest via `SELF.fetch` + `fetchMock`; the browser suite fixtures the LIFF SDK **and the Worker's `/api/adapters/line/*` responses** at the Playwright layer (no server-side LINE call is ever triggered from CI) while asserting zero non-fixture browser egress. The fixture secret reaches the webServer as `--var LINE_MESSAGING_CHANNEL_SECRET:...` (the existing `--var` pattern, not process env) |
| Docs & release | `docs/PARITY.md`, `docs/ROADMAP.md`, new `docs/LINE-SETUP.md`, `docs/PRIVACY.md`, `docs/RELEASING.md` (rollback/deployment note, decision 4), `public/privacy.html`, `release/public-files.txt`, `scripts/release-audit.mjs` | Status flips with evidence (SC-005); operator walkthrough (Mini App recommended — unverified ones are open to individuals in Japan — LINE Login channel equally supported; **both channels under one provider**, stated as a non-machine-checkable constraint; Official Account quota: free plan = 200 push messages/month, delivery stops at the cap; deactivation order); privacy surfaces enumerate stored subject, per-message facts, retention, unlink; allowlist + REQUIRED registration |
...
4. **Rollback-safe storage via `__`-prefixed tables — no schema-set change at all**: both DOs'
   ...
   a rollback removes its class — adapter personal data (links, subjects) could then outlive
   the parent retention. The deployment note in `docs/RELEASING.md` therefore gates it: **a
   rollback across this feature's introduction requires `disabled` plus zero personal/event
   rows in the authority store** (the non-identifying generation high-water persists by
   design) — the redacted ledger and webhook-dedup rows have passed their TTL — **there is no erase
   command** (an earlier draft had one; it grew a saga of its own and was cut — TTL waiting is
   the whole mechanism). Diagnostics show the remaining TTL and flip to "no personal or event
   data retained" when the store drains. Two properties make the wait sufficient: the
...
6. **Link model** (reservation-scoped; management key is the customer-held proof):
   ...
   - **No retroactive delivery, decided by a clock-free watermark in one revision domain, with
     ...
     `webhookEventId` — no subject stored.
...
Every state above is browser-tested, including
the transitions (inactive → active, active → missing-secret → deactivating → purged) in one
browser context — with `Cache-Control: no-store` (the existing helper) on every state-dependent
response (`/api/config`, the module, LIFF aliases, `privacy.html`) so a cached 200/404 cannot
leak across states (direct-fetch assertions included).
...
| Identity: retry/terminal visibility | Bounded in-flow retry; link-failure count in diagnostics |
