...
| Docs & release | `docs/PARITY.md`, `docs/ROADMAP.md`, new `docs/LINE-SETUP.md`, `docs/PRIVACY.md`, `docs/RELEASING.md` (backout/deployment note, decision 4), `public/privacy.html`, `release/public-files.txt`, `scripts/release-audit.mjs` | Status flips with evidence (SC-005); operator walkthrough (Mini App recommended — unverified ones are open to individuals in Japan — LINE Login channel equally supported; **both channels under one provider**, stated as a non-machine-checkable constraint; regional Official Account quota warning, including the Japan Communication Plan example of 200 push messages/month with delivery stopping at the cap; deactivation order); privacy surfaces enumerate stored subject, per-message facts, retention, unlink; allowlist + REQUIRED registration |
