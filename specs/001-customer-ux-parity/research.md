...
- **D2 — Residual gaps** (issue checklist → status):
  ...
  - Duplicate-booking warning driven by the backend — **gap; client-side reuse of the existing per-reservation status endpoint** (see D4).
...
- **D4 — Duplicate-signal design (privacy-first)**: matching by free-text contact would create a contact-enumeration oracle. Instead the check is proof-bound: the browser already stores owned-booking records (reservationId + management key) for up to a year (`journey.js`), and `POST /api/reservations/:id/status` already verifies that proof today (the bookings page calls it). Before submission, the client checks its *remembered same-day reservations* (capped at 3 lookups) against that existing endpoint and warns when any is still active. Zero backend change, no way to probe other people's contacts. Cross-device duplicates are out of scope and recorded as such in the parity matrix (an identity adapter is the future cross-device answer, per issue #1).
