...
| Descriptor timeout or lease expiry could commit a mutation without recovery evidence | The fail-open path omitted the calendar descriptor, so the day wrote no calendar outbox row and an ordinary sweep had nothing to drain | **Fixed.** The same transaction writes an unbound generation-0 calendar event with a day-global sequence; the active authority adopts it, and a lifecycle race leaves it unacknowledged for retry. |
...
| Revoked shared credentials could drive one provider call per queued row (`CWE-400`) | A configuration result parked only the claimed row, so other rows remained sendable in the same or next alarm | **Fixed.** The authority persists the rejected non-secret fingerprint, parks current and newly accepted work together, stops the batch, and reopens it only on rotation or explicit reconciliation. |
...
the pinned GitLab SSRF rule reported 0 findings; 25 focused security regressions passed; 77
