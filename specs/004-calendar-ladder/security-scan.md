...
| Wrong feed tokens could force unthrottled authority writes (`CWE-770`) | The initial feed path reached the authority without a limiter and activated the descriptor before rejecting the token | **Fixed and suppressed.** The Worker applies `PUBLIC_RATE_LIMITER` before namespace access and preserves a uniform 404; the authority validates and constant-time compares the token before `descriptor()`, initializing only bounded counter storage so the first configured-mode failure is retained. |
| Public privacy reads could force or stall on residual Calendar DO lookups (`CWE-770`, `CWE-400`) | With both local modes absent, every request queried `hasDisclosure()` and an unresolved RPC had no deadline | **Fixed and suppressed.** The residual lookup now passes through `PUBLIC_RATE_LIMITER` and the shared 250 ms authority deadline; a limited request performs no namespace access, and a limited, failed, or timed-out lookup conservatively renders bounded conditional disclosure so cleanup state cannot be hidden. |
...
| A full mutation table could partially replace or falsely complete one reconciliation date | A failed required delete could leave sibling writes, while a failed required upsert was ignored before the date watermark advanced | **Fixed.** One bounded preflight defers the entire date before any projection write unless every required Google upsert and delete fits; the write path asserts that invariant. |
...
| OAuth or an earlier Calendar response could let the same item cross retention before its next request | The claim checked retention only once, so an initial mutation after OAuth or a PUT→POST→PUT fallback could begin after the parent boundary and create an untracked external event | **Fixed.** The claimed `purgeAt` reaches the shared sender and is checked immediately before every DELETE, PUT, and POST; expired work returns to the existing retention prune without another provider call. |
| Descriptor timeout, lease expiry, or a late recovery commit could lose evidence or recreate data after disable | The fail-open path originally wrote no calendar outbox row; its later recovery marker then had an effectively infinite lease, so a delayed commit could recreate generation-0 data after the final purge | **Fixed.** Timely recovery writes one unbound generation-0 event with a day-global sequence for the active authority to adopt. Timeout recovery is bounded to 30 seconds, ordinary expired descriptors stop at the 60-second final-pass bound, and later commits omit only the optional event. |
