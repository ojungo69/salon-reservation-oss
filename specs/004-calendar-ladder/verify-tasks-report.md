...
| `npx vitest run test/calendar-adapter.test.ts test/reservation-day.test.ts --reporter=verbose` | 78/78 passed (calendar 52; reservation-day 26) |
...
| focused security regression command in `security-scan.md` | 29/29 passed |
| focused calendar privacy cleanup test | 1/1 passed |
| `npm run check` | core 54/54; Workers/DO 229/229; typecheck; generated types; Wrangler dry-run build; npm audit 0; release audit 77 files |
...
A final retention review then reproduced a successful Google upsert whose mutation row disappeared
before local retention pruned its projection, leaving the provider event orphaned. The active sweep
now queues Google deletion during the final 12 hours, retains ICS until the source boundary, and
marks successful cleanup so credential rotation cannot recreate the event. The regression failed
with only `PUT` and now observes `PUT` then `DELETE`; the 78-case focused, 229-case full, and 29-case
security runs pass.
...
| T012 | Projection/dedup/order/retention/overflow/feed authority tests pass in the 52-case calendar suite. |
...
| T034 | 78-case focused, 229-case full, type/build/audit, and 34-case browser results are recorded above. |
