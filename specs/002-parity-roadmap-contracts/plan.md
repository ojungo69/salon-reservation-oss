...
scripts/release-audit.mjs    # + the same two paths in the REQUIRED set (deletion fails the audit)
```
...
7. **`release/public-files.txt` and `scripts/release-audit.mjs`** — add the two new docs to the manifest in lexicographic order, and register the same two paths in the audit script's `REQUIRED` set so a later deletion (even together with its manifest entry) fails `release:audit` instead of passing silently.
